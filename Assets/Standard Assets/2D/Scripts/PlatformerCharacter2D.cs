...
namespace UnityStandardAssets._2D {
	public class PlatformerCharacter2D : MonoBehaviour {
		...
		[SerializeField]
		private float m_LadderClimbSpeed = 8f;
		[SerializeField]
		...
		void NormalizeSlope() {
			if ( m_Grounded ){
				...
				RaycastHit2D hit = Physics2D.Raycast(this.transform.position, -Vector2.up, Mathf.Infinity, m_WhatIsGround);
				if ( hit && Mathf.Abs(hit.normal.x) > 0.1 ) {
					//Apply the opposite force against the slope force 
					//Reduce normal factor by 0.6 to deal with surface's friction
					m_Rigidbody2D.velocity -= new Vector2(hit.normal.x * 1f, 0);
        
					//Move Player up or down to compensate for the slope below them
					this.transform.position += new Vector3(0, -hit.normal.x * Mathf.Abs(m_Rigidbody2D.velocity.x) * Time.deltaTime * (m_Rigidbody2D.velocity.x - hit.normal.x > 0 ? 1 : -1), 0);
				}
			}
		}
	}
